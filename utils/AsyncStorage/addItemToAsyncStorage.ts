        ...
        if (existingItems) {
            ...
            if (itemId) {
                ...
                if (index !== -1) {
                    ...
                    await editItemExpiry(itemId, itemName, expiryDate)
                } else {
                ...
                await scheduleExpiryNotification(nextId, itemName, expiryDate)
            }
        } else {
            items.push({ id: 1, name: itemName, date: expiryDate })
            await scheduleExpiryNotification('1', itemName, expiryDate)
        }
